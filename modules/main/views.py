...
class VideoBlockView(TemplateView):
    template_name = "components/video_block.html"

    def get_context_data(self, **kwargs):
        context = super(VideoBlockView, self).get_context_data()
        context.update({
            "embed_video": Video.objects
                                .filter(id=self.request.GET.get('video_id'))
                                .first()
        })
        return context


show_video = VideoBlockView.as_view()
